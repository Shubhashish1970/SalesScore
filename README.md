...
- **Secrets:** Ensure **`GEMINI_API_KEY`** is set in GitHub (Settings → Secrets). The same **`FIREBASE_SERVICE_ACCOUNT_JSON`** (or **`GCP_SA_KEY`**) used for Firebase deploy is used to deploy the function; the service account needs **Cloud Functions Developer** and **Service Account User** in your GCP project.
