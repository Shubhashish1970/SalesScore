...
All types live in `src/types/scorecard.ts`. **Config** (bands, thresholds, weights) is in `src/lib/app-config.ts` and will be editable via the Admin Settings screen.
